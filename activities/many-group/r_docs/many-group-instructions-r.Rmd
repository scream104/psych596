...
- between subjects ANOVA (1 factor, 3 levels): conduct a one-way analysis of variance (one ) with `lm()` and `anova()` using the default treatment/dummy coding scheme  
    ...
    - non-parametric alternative: equal variance not assumed using `oneway.test()` function  
...
- **Earlier you downloaded [oveis-etal-2020.csv](../data/oveis-etal-2020.csv)**   
...
The anova table now gives us an F-statistic for each predictor. The F-stat for the emotion regulation condition now tells us about the difference in group means, adjusting for closeness. The F-stat for closeness (`prepIOS_cent`) tells us about the covariance with the DV. You can examined planned contrasts, which are adjusted for the covariate, or compare adjusted means using post-hoc tests that we will discuss in the final step.  
