...
- install the required packages: 
    ...
    - `install.packages("optimx")` *note: students with a particular R version around 4.2.2 may not be able to install this package - this is only needed for one step so you can skip it   
...
- start a new R markdown for your code and notes and include the usual code in the setup chunk, but add `library(lme4)` to the setup chunk (and then run the chunk)   
    - do you get an error along the lines of "cholmod_factor_ldetA not provided by package Matrix" when you try to load the package?  if yes, try   
...
- `modality` is systematically varied so we consider it a fixed effect. In designs (e.g., correlation) where no variable is systematically manipulated we can consider a fixed effect to be one where all levels of interest are covered (e.g., `age` in the lumosity data we used long ago). 
- If the levels of a variable are a sample of a larger set we can consider it a random effect (e.g., participants sampled from a population, or a set of words sampled from a language)   
