---
title: "Moderation and Mediation Models"
...
library(tidyverse)
source('~/Documents/Teaching/Statistics-Graduate-596/www/psych596/activities/moderation-mediation/r_docs/process.R')
```
...
- **Earlier you downloaded [lumos_subset1000pluseye.csv](../data/lumos_subset1000pluseye.csv)**     
...
cat("Variance Inflation Factor Table:\n")
...
- If we subtract the mean age from each value of `age` and store it in a new variable called `age_cent` (and do the same for `imaginary_screensize`), then we can enter these new variables into the regression instead. Then we will be able to interpret the resulting main effect coefficients (as, e.g., the effect of age at the mean value of screen size).
...
- Then re-estimate a new model (store it in a variable called `score_ageXscreen_cent_lm`) that is the same as the last one except you use the centered variables (and interaction of the centered variables) as the predictors. Use `summary()` to get the model summary and `car::vif()` to get multicollinearity info. 
...
Forget about the screen size variable for a moment and consider possible explanations for the negative relation between age and performance that we saw when `age` was our only predictor for `raw_score`. Maybe part of the relation could be explained by something like eyesight that deteriorates with age. The data file you imported for this activity has a new (simulated/imaginary) variable called `eyesight_z` which is an eyesight "score" where higher values indicate better eyesight (it has been scaled such that the sample mean is 0 and the s.d. is 1). We will test a mediation model where `eyesight_z` explains the relation between `age` and `raw_score`.
*A note about causality:* In this model we have good reasons for thinking the direction of the relationships is as specified in the mediation model (i.e., it would not be possible for a change in eyesight to cause a change in age, or for a change in test performance to cause a change in eyesight) but there may be many unmeasured variables that could be involved. The test of our mediation model will tell us whether the `eyesight_z` measure accounts for a "significant" part of the relationship between `age` and `raw_score`. Here are the four conditions for our mediation model test, which are tested by with three regression models. **First we will list the regression models (coefficients of each model are different so we'll refer to them each with unique subscripts instead of the usual *b*<sub>0</sub>, *b*<sub>1</sub> terminology):**   
...
- We will use the `process()` function in an R script developed by Andrew Hayes to test our mediation model. Unlike all the functions we have used before, this function is not contained in an R package, so we can't use `install.packages()`. Instead, we need to download an R-script that has the source code for the `process()` function, then *source* the script (*source* just means run within your current environment) to make the function available within your project environment. Here is how to do that:  
...
- the section starting with `Outcome Variable: raw_score` corresponds to **model #3** of the three models we use to test the mediation. 
...
- now look at the section starting "TOTAL, DIRECT, AND INDIRECT EFFECTS OF X ON Y"  
    ...
    - the "effect" estimage for the "Completely standardized indirect effect ..." is a standardized form of the a\*b coefficient, meaning an increase of one standard deviation in `age` predicts that much decrease in `raw_score` through the indirect path only  
