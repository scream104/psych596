...
#### 3.3 Recompute the confidence interval using bootstrapping (repeated resampling with replacement from the original sample)
- just add it into the table that you generated in step 3.1b   
- use the `ggplot2::mean_cl_boot()` function (nonparametric bootstrap w/ 1000 iterations) in the same way that you previously used `ggplot2::mean_cl_normal()` (see the solution below)  
- what do you notice about the confidence interval around the mean?  
<button class="btn btn-primary" data-toggle="collapse" data-target="#Block3-3"> Code to add bootstrapped 95% confidence interval around mean</button>
<div id="Block3-3" class="collapse">    
add 95% confidence intervals using ggplot2::mean_cl_boot()  
```{r Step3.3-nhanes-height-all, fig.show='hold', results='hold'}
nhanes_tib %>% dplyr::summarise(
  median =  median(Height,na.rm = TRUE),
  mean =  mean(Height,na.rm = TRUE),
  norm.ci.low = ggplot2::mean_cl_normal(Height)$ymin,
  norm.ci.upp = ggplot2::mean_cl_normal(Height)$ymax,
  boot.ci.low = ggplot2::mean_cl_boot(Height)$ymin,
  boot.ci.upp = ggplot2::mean_cl_boot(Height)$ymax,
  sd = sd(Height,na.rm = TRUE),
  cases = n() - sum(is.na(Height))
  ) %>% 
  knitr::kable(caption = "Height Descriptives- all individuals", digits = 3) %>% 
...
#### 3.4 Filter by Age 
-   Insert a new code chunk (call it *"Step3.4-nhanes-height-adults"*)  
...
```{r Step3.4-nhanes-height-adults, fig.show='hold', results='hold'}
# code for table
...
#### Step 5.1 - Use `pivot_longer()` to restructure the data to tidy format 
