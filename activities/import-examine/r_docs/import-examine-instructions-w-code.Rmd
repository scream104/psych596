...
- use `mean()`, `median()`, and `sd()` functions and select the "Height" column using "\$" like this  `mean(nhanes_tib$Height, na.rm = True)` (notice what happens if you leave out "na.rm = True"). It's a good idea to also count missing values - use `n()` and `sum(is.na())`   
...
```{r Step3.1-nhanes-height-all, fig.show='hold'}
...
<button class="btn btn-primary" data-toggle="collapse" data-target="#Block3-1b"> Want to go further? Add a 95% confidence interval around the mean</button>  
<div id="Block3-1b" class="collapse">    
Bonus - add 95% confidence intervals using the formula (same as spss calculation): mean +/- qt(0.975,df=n-1) \* sd/sqrt(n_obs) -- *alternatively, you could find a package that has a confidence interval function*  
```{r Step3.1bonus-nhanes-height-all, fig.show='hold'}
n_obs <- nhanes_tib %>% drop_na(Height) %>% nrow()
nhanes_tib %>% drop_na(Height) %>% dplyr::summarise(
    median =  median(Height),
    mean =  mean(Height),
    ci.low = (mean(Height) - (qt(0.975,df = n_obs-1)*sd(Height)/sqrt(n_obs))),
    ci.upp = (mean(Height) + (qt(0.975,df = n_obs-1)*sd(Height)/sqrt(n_obs))),
    sd = sd(Height),
    cases = n()
    ) %>% 
    knitr::kable(caption = "Height Descriptives- all individuals", digits = 3) %>% 
...
```{r Step3.2-nhanes-height-all, fig.show='hold'}
...
```{r Step3.3-nhanes-height-adults, fig.show='hold'}
...
cort_tib %>%  dplyr::summarise(
    ...
    cases = n(),
    missing = sum(is.na(cortisol_baseline))
    ) %>% 
...
```{r Step4.3-log-cort-distribution, fig.show='hold'}
cort_tib <- cort_tib %>% 
...
cort_tib %>%  dplyr::summarise(
    ...
    cases = n(),
    missing = sum(is.na(ln_cortisol_baseline))
    ) %>% 
...
```{r Step5-nhanes-height-challenge, fig.show='hold'}
